...
| `labels` _object (keys:string, values:string)_ | Additional labels for the StatefulSet and Pod. The selector labels<br />app.kubernetes.io/name, app.kubernetes.io/instance, and<br />app.kubernetes.io/managed-by are reserved by the operator. |  | Optional: \{\} <br /> |
| `annotations` _object (keys:string, values:string)_ | Additional annotations for the StatefulSet and Pod. Keys in the<br />ha.homeassistant.io domain are reserved by the operator. |  | Optional: \{\} <br /> |
